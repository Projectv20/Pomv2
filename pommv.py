...
class BotManager:
    ...
    async def _run_bot(self, token: str):
        try:
            ...
            # Verify token works with Telegram server
            ...
            # Clear conflicting webhooks to ensure polling stays active
    ...
    async def restart(self, token: str):
        ...
        if token.strip():
            ...
            # Yield to let polling initialize
...
# --- Callback Router (Button Clicks & Admin Approval in Bot) ---
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # In-Bot Admin Order Verification Actions
...
# --- Media Router: Admin Upload Mode or User Payment Proof ---
async def handle_incoming_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if session and msg.photo:
        ...
        if cfg["admin_chat_id"]:
            try:
                # Find matching order id from database
                conn = sqlite3.connect(DB_NAME)
                c = conn.cursor()
                ...
                # In-Bot Accept / Reject inline keyboard
...
async def admin_dashboard_view(request: Request, message: str | None = None):
    ...
    # Manage Users Table with Subscription Control & Ban Toggle
    users_rows = ""
    ...
    # Render All Dynamic Plan Button Cards
    pack_cards_html = ""
    num_buttons = len(cfg["buttons"])
    for idx in range(num_buttons):
        b_name = cfg["buttons"][idx]
    ...
    # Always-Online badge calculation
...
<body class="text-slate-100 min-h-screen flex overflow-x-hidden relative">
    ...
    <main class="flex-1 flex flex-col min-w-0 h-screen overflow-y-auto relative z-10">
        ...
        <div class="p-4 md:p-8 max-w-5xl w-full mx-auto space-y-6">
            ...
            <!-- TAB: MANAGE USERS (WITH SUBSCRIPTION CONTROL & BAN TOGGLE) -->
            ...
            <!-- TAB: BROADCAST (TEXT, PHOTO, VIDEO) -->
            ...
            <!-- TAB: PLAN BUTTONS (WITH CHOOSE FILE, ACCESS LINK & ADD BUTTON) -->
